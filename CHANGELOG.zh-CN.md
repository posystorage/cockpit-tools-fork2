...
- **模型指纹测智**：同时多选 OAuth 账号、独立 API Key 账号和中转站供应商的各个 API Key，按账号／上游 × 请求模型组合采样。支持 GPT-5.5、GPT-5.6 Sol/Terra/Luna、GPT-6 Astra/Sol/Luna，首次只勾选 GPT-5.6 Sol、GPT-6 Sol、GPT-6 Astra。
- **选择记忆与批量控制**：首次账号全不选，后续恢复上次启动测试的账号；模型、并行数和采样数编辑后记忆。最大并行数 1–10、默认 3，每组采样可选 1／2／3、默认 3，支持停止在途请求及队列、最小化后继续测试。
- **本地指纹归因与逐组结果**：内置固定版本的 ModelTrace 算法和 16 模型参考库，展示请求模型、上游返回模型字段、统计候选概率、有效样本数和原始回答；样本不足明确标为“部分完成”，请求失败不自动重试或更换账号／模型。分析不向 ModelTrace 上传凭据或回答，偏好不保存 API Key，结果概率不作为模型身份的确定证明。
- **独立传输与取消**：OAuth 使用 API Service 的指定账号通道并为每个样本创建独立会话；API 上游支持 Responses 和 Chat Completions，包含超时、响应校验及错误脱敏。

### 变更

- **统一“指纹测智”入口**：账号概览页按钮紧邻“鹈鹕测智”左侧；模型供应商页按钮紧邻“一键测试”左侧，两处共用同一测试面板。
- **更新 fork 维护与发布约定**：补充指纹测智的合并保护项、核心文件、默认值、记忆规则、资产许可与验收清单；`1.3.59b3` 草稿说明只包含 b2 到 b3 的增量，继续构建 Windows、macOS Apple Silicon、Intel 和 Universal。本版不修改额度耗尽提示隐藏逻辑。
...
- **按实例隐藏 Codex Desktop 原生额度耗尽提示**：API Service 桌面启动默认只隐藏经语义确认的 `workspace_member_credits_depleted` 提示，其他绑定默认关闭，CLI 启动不注入显示修改；默认实例和普通实例都可在启动预览中保存显式选择。未知、不可读、语义冲突或无关提示保持可见，关闭开关或会话结束会恢复原始 inline 样式。该功能只改变显示，不修改请求、调度、token、额度或 Codex 行为。

### 变更

- **移除 fork 的 Turn 票据监听与显示**：不再采集、保存或展示 292/312/332/356 等 `X-Codex-Turn-State` 长度及最近观察时间，普通 Codex 账号卡与 API 服务账号池卡均移除相关 UI。上游为官方请求兼容保留的空请求头不属于监听功能。
...
- **只读监听 Codex Turn-State**：API 服务启用期间，按本地 OAuth 账号分别记录上游响应头 `X-Codex-Turn-State` 长度为 292、312、332、356 时最近一次观察的时间与模型。普通 Codex 账号卡和独立 API 服务账号池卡都显示四种长度；账号移出服务后，普通卡保留既有时间并标注监听关闭。这些数字只是 opaque 响应头的长度，不是 HTTP 状态码，也不证明令牌有效；监听器不主动探测、不注入或替换请求头、不改变调度，也不保存原始状态值。
- **保护 Turn-State 诊断数据**：sidecar 排除内部 API Key 请求，Rust 再核验账号池成员与密钥作用域，上游调试日志对该响应头完整脱敏。观测元数据与计费日志分开存放，并在后台读取和写入，不阻塞请求或服务启动。
...
- **分离 Codex 上下文设置与模型管理**：普通账号默认跟随官方目录，明确确认后才启用 Cockpit 模型目录；独立的上下文管理提供跟随官方、`516K / 460K`、`1M / 900K` 和自定义选项，并清理旧单模型上下文覆盖。
- **Codex 可见模型支持自定义排序**：可拖拽调整顺序，新模型插入相同来源模型之后。
- **保留 Business 账号的 Team 历史额度**（[#2293](https://github.com/jlcodes99/cockpit-tools/pull/2293)）：切换按量计费后按用户与工作区隔离展示历史额度及重置时间，并标记为本地快照。

### 修复

- **兼容 Homebrew 6 Cask 安装流程**（[#2279](https://github.com/jlcodes99/cockpit-tools/pull/2279)）：隔离属性清理迁移至 `postflight_steps`，移除弃用的 URL 验证参数。
- **修复 WebSocket 长连接的分次用量统计**（[#2292](https://github.com/jlcodes99/cockpit-tools/pull/2292)）：逐次记录和去重用量并保留账号归属。
- **修复 Antigravity GCP ToS 账号切换资格与状态丢失**（[#2286](https://github.com/jlcodes99/cockpit-tools/pull/2286)）：补齐元数据、保留 IDE 缓存并向现有数据目录同步凭据。
- **修复 Codex API 服务与账号池恢复**：加入受控的 Sidecar 进程恢复、端口清理保护和不等待持久化写入的逐账号进度。Sidecar 进程监控及端口清理变更在 1.3.44 回退，以该版说明为最终行为。
- **修复第三方 API 路由无法关闭**：保留渠道配置并持久化明确的禁用状态。
- **修复混合路由下即时语音不可用**：通话建立与实时控制共用本地网关和账号，退出时不提前恢复仍在运行的实例。
- **恢复第三方 GPT 模型的 Fast 和 Ultra 档位**，保留保存的模型顺序。
- **保留模型路由草稿与提示**：刷新账号不覆盖未保存配置，允许保存空模型渠道。
