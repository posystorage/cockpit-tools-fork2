...
- 合并本轮上游 `1.3.41` 与 `1.3.42` 的全部变更，不重复聚合 `1.3.40` 及更早版本。
- 接受上游 `rolling7d` 作为默认近 7 天统计范围，并保留 API Service 独立页面的近 24H、近 48H 选项；旧 `last7d` 偏好自动迁移为 `rolling7d`，普通共享弹窗不显示 24H/48H。
- 保留 API Service 调度观测、全部账号列表、调度优先排序和账号过多时的内部滚动；`recordingSelector` 继续覆盖完整上游选择器链并记录 affinity cache-hit。
- 保留删除账号历史计费、GPT-5.6 Luna 计费口径、Sub2API 用量兼容、去广告/远端配置关闭，以及 Windows + macOS 草稿构建边界。

### 修复

- **修复 fork 草稿包的跨平台 Rust 编译**：移除 API Service 账号保存函数参数列表中合并遗留的多余泛型结束符；该语法错误曾使 `1.3.42b1` 的 Windows 与三个 macOS 构建均在产物生成前失败，`1.3.42b2` 重新执行完整平台构建。
...
- **普通 Codex 账号页恢复所有现存账号的窗口用量**：查询范围不再受 API 服务账号池成员身份限制；账号移出 API 服务后仍显示真实请求数、Token 与 `A $`，只有已删除账号继续仅在 API 服务历史区显示。
- **独立 Codex API 服务统计新增滚动范围**：新增近 24H、近 48H 和近 7Day，按当前时刻滚动并持续刷新；共享管理弹窗保持原有日/月/周范围。
...
- **Codex API 服务新增历史账号统计**：账号池与按账号统计区分当前账号、已移出账号池的“未加入”账号和已经删除的账号；历史请求数、Token 与估算费用继续保留，并保持原账号类型 Tag 与历史状态 Tag 的明确区分。
- **最低优先级账号支持暂停兜底调度**：独立 API 服务页和普通 Codex 摘要卡片共享同一开关，暂停账号继续留在账号池但不会被 legacy 或 sidecar 网关选中；恢复时保留已有模型排除规则。

### 变更

- **Codex 设置改为扁平布局并即时保存**：配置文件、上下文预设、自定义值、实验模型、切号联动、额度显示和自动切号都与外层设置保持同级；点击预设或开关、编辑字段后直接持久化，不再需要单独的保存或刷新按钮，连续修改也会按顺序保存。
- **更新 GPT-5.6 Terra/Luna 本地估算价格基线**：价格表版本升级为 v3，Terra 默认输入/缓存输入/输出价格调整为 `2.00 / 0.20 / 12.00` 美元每百万 Token，Luna 调整为 `0.20 / 0.02 / 1.20`；长上下文与 Fast 倍率继续沿用既有规则，旧价格历史请求在后台重算。
- **强化兜底暂停的状态一致性**：开关改为单账号后端更新；暂停账号改为正常或最高优先级时自动清除暂停通配符但保留其他模型规则；“禁用模型”整表编辑增加版本校验，避免旧草稿覆盖其他页面刚保存的调度状态。
...
- **修复“模型与能力”价格设置因可选预填值为空而无法保存的问题**：缓存、长上下文、Priority 价格和 Token 阈值可以按模型能力留空；仅对实际填写的价格和阈值执行数值校验。
