...
func (s *recordingSelector) Pick(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, auths []*coreauth.Auth) (*coreauth.Auth, error) {
	...
	if s.emitter != nil {
		...
		requestKind, _ := ctx.Value(requestKindContextKey).(string)
		if requestKind == "" { requestKind = requestKindFromPath(internallogging.GetEndpoint(ctx)) }
		requestModel, _ := ctx.Value(requestModelContextKey).(string)
		if strings.TrimSpace(requestModel) != "" { model = requestModel }
		account := accountForAuthInManifest(s.manifest, auth)
		routingStrategy := ""
		if s.manifest != nil { routingStrategy = strings.TrimSpace(s.manifest.RoutingStrategy) }
		s.emitter.emit(requestDiagnosticPayload{
			...
			CandidateAuths: len(auths), AvailableAuths: len(auths), RoutingStrategy: routingStrategy,
			Provider: provider, AuthID: auth.ID, AccountID: stringFromAccount(account, "id"), AccountEmail: stringFromAccount(account, "email"),
		})
	}
	return auth, nil
}
...
func buildCoreAuthManager(cfg *config.Config, selector coreauth.Selector, hook coreauth.Hook, m *manifest, quota *quotaReserveStateStore, tracker *requestUsageTracker, emitters ...*eventEmitter) *coreauth.Manager {
	...
	if tracker != nil {
		...
		selector = &recordingSelector{inner: selector, manifest: m, tracker: tracker, emitter: emitter}
	}
