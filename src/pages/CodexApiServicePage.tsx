...
export function useCodexApiServicePageController() {
  ...
  const selectedStatsWindow =
    useMemo<CodexLocalAccessStatsWindow | null>(() => {
      ...
      if (statsRange === "daily" || statsRange === "weekly" || statsRange === "monthly") {
  ...
  const memberAccounts = useMemo(
    () => {
      const activity = new Map((state?.accountActivity ?? []).map((item) => [item.accountId, item]));
      return memberIds
        .map((accountId) =>
        ...
        .sort((left, right) => {
          const a = activity.get(left.id);
          const b = activity.get(right.id);
          return (b?.runningCount ?? 0) - (a?.runningCount ?? 0)
            || Math.max(b?.lastFinishedAt ?? 0, b?.lastSelectedAt ?? 0) - Math.max(a?.lastFinishedAt ?? 0, a?.lastSelectedAt ?? 0)
            || left.email.localeCompare(right.email);
        });
    },
    [memberIds, localAccessAccounts, state?.accountActivity],
  );
  const activityByAccountId = useMemo(
    () => new Map((state?.accountActivity ?? []).map((item) => [item.accountId, item])),
  ...
  useEffect(() => {
    if (!collection?.enabled) return undefined;
    const timer = window.setInterval(() => {
      void reloadState();
    }, 5000);
    return () => window.clearInterval(timer);
  }, [collection?.enabled, reloadState]);
  ...
  const selectedStatsRangeTitle =
    statsRange === "daily"
      ...
      : statsRange === "weekly"
        ...
        : statsRange === "last24h"
          ? t("codex.apiService.statsRange.last24h", "近 24H")
          : statsRange === "last48h"
            ? t("codex.apiService.statsRange.last48h", "近 48H")
            : statsRange === "last7d"
              ? t("codex.apiService.statsRange.last7d", "近 7Day")
