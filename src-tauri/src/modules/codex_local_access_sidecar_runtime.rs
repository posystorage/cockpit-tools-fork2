...
async fn record_sidecar_usage_event(event: SidecarUsageEvent) {
    finish_account_activity(
        non_empty_sidecar_string(&event.request_id).as_deref(),
        non_empty_sidecar_string(&event.account_id).as_deref(),
        non_empty_sidecar_string(&event.account_email).as_deref(),
        non_empty_sidecar_string(&event.api_key_id).as_deref(),
        non_empty_sidecar_string(&event.api_key_label).as_deref(),
        non_empty_sidecar_string(&event.model).as_deref(),
        ...
        "auth_selected" => match serde_json::from_value::<SidecarAuthSelectedEvent>(value) {
            Ok(event) => {
                record_account_activity_selected(RequestActivitySelection {
                    ...
                    email: event.account_email,
                    api_key_id: event.api_key_id,
                    api_key_label: event.api_key_label,
                    model_id: event.model,
                    request_kind: parse_sidecar_request_kind(&event.request_kind),
                    routing_strategy: event.routing_strategy,
                }).await;
            }
            Err(error) => logger::log_codex_api_warn(&format!("[CodexLocalAccess] sidecar auth_selected 事件解析失败: {}", error)),
        },
