...
pub async fn update_local_access_backup_dispatch(account_id: String, enabled: bool) -> Result<CodexLocalAccessState, String> {
    ensure_runtime_loaded().await?;
    let mut collection = gateway_runtime().lock().await.collection.clone().ok_or_else(|| "本地接入集合尚未创建".to_string())?;
    if !set_backup_dispatch_enabled(&mut collection, &account_id, enabled)? { return snapshot_state().await; }
    collection.updated_at = now_ms();
    save_collection_to_disk(&collection)?;
    { let mut runtime = gateway_runtime().lock().await; sync_runtime_collection(&mut runtime, collection); }
    ensure_gateway_matches_runtime().await?;
    snapshot_state().await
}
