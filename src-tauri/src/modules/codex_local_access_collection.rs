...
fn set_backup_dispatch_enabled(collection: &mut CodexLocalAccessCollection, account_id: &str, enabled: bool) -> Result<bool, String> {
    let account_id = account_id.trim();
    if account_id.is_empty() || !collection.account_ids.iter().any(|id| id == account_id) { return Err("API 服务账号不存在".to_string()); }
    if !collection.custom_routing_rules.iter().any(|rule| rule.account_id == account_id && rule.is_backup) { return Err("仅最低优先级账号可设置兜底调度".to_string()); }
    let before = collection.account_model_rules.clone();
    if let Some(rule) = collection.account_model_rules.iter_mut().find(|rule| rule.account_id == account_id) {
        rule.excluded_models.retain(|model| model.trim() != "*");
        if !enabled { rule.excluded_models.push("*".to_string()); }
    } else if !enabled {
        collection.account_model_rules.push(CodexLocalAccessAccountModelRule { account_id: account_id.to_string(), excluded_models: vec!["*".to_string()] });
    }
    collection.account_model_rules = normalize_account_model_rules(collection.account_model_rules.clone(), &collection.account_ids);
...
fn sanitize_collection_structure(
    ...
    if collection.model_pricing_book_version < DEFAULT_MODEL_PRICING_BOOK_VERSION {
        let before = collection.model_pricings.len();
        collection.model_pricings.retain(|pricing| !is_superseded_default_56_pricing(pricing));
        if collection.model_pricings.len() != before { changed = true; }
        collection.model_pricing_book_version = DEFAULT_MODEL_PRICING_BOOK_VERSION;
        collection.model_pricing_version = collection.model_pricing_version.max(DEFAULT_MODEL_PRICING_VERSION).saturating_add(1);
        changed = true;
