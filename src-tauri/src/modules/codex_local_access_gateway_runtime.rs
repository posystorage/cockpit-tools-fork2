...
async fn record_request_stats_with_meta(
    ...
    let legacy_gateway = {
        ...
        runtime
            .collection
            .as_ref()
            .is_some_and(|collection| collection.gateway_mode == CodexLocalAccessGatewayMode::Legacy)
    };
    if legacy_gateway && account_id.is_some() {
...
fn normalize_activity_request_id(value: Option<&str>) -> String {
    value.map(str::trim).filter(|v| !v.is_empty()).unwrap_or_default().to_string()
}
...
fn prune_account_activity(runtime: &mut GatewayRuntime, now: i64) {
    ...
    runtime.account_activity.retain(|account_id, item| {
        let running = runtime.running_requests.values().any(|request| request.account_id == *account_id);
        let recent = item.last_finished_at.unwrap_or(item.last_selected_at);
        running || now.saturating_sub(recent) <= ACCOUNT_ACTIVITY_RECENT_WINDOW_MS
    });
}

fn record_account_activity_selected_locked(runtime: &mut GatewayRuntime, selection: RequestActivitySelection, now: i64) {
    if selection.account_id.trim().is_empty() { return; }
    let request_id = normalize_activity_request_id(Some(&selection.request_id));
    if request_id.is_empty() { return; }
    let request = CodexLocalAccessRunningRequest {
        request_id: request_id.clone(), account_id: selection.account_id.clone(), email: selection.email.clone(),
        api_key_id: selection.api_key_id.clone(), api_key_label: selection.api_key_label.clone(), model_id: selection.model_id.clone(),
        request_kind: selection.request_kind, routing_strategy: selection.routing_strategy.clone(), started_at: now, last_seen_at: now,
    };
    runtime.running_requests.insert(request_id.clone(), request);
    let entry = runtime.account_activity.entry(selection.account_id.clone()).or_insert_with(|| CodexLocalAccessAccountActivity {
        account_id: selection.account_id.clone(), email: selection.email.clone(), ..Default::default()
    });
    if !selection.email.trim().is_empty() { entry.email = selection.email; }
    entry.last_selected_at = now;
    entry.last_model_id = selection.model_id;
    entry.last_api_key_id = selection.api_key_id;
    entry.last_api_key_label = selection.api_key_label;
    entry.last_request_kind = selection.request_kind;
    entry.routing_strategy = selection.routing_strategy;
    entry.recent_request_id = request_id;
}
...
fn finish_account_activity_locked(runtime: &mut GatewayRuntime, request_id: Option<&str>, account_id: Option<&str>, account_email: Option<&str>, api_key_id: Option<&str>, api_key_label: Option<&str>, model_id: Option<&str>, request_kind: CodexLocalAccessRequestKind, now: i64) {
    let mut request = request_id.and_then(|id| runtime.running_requests.remove(id.trim())).filter(|item| !item.account_id.trim().is_empty());
    if request.is_none() {
        if let Some(account_id) = account_id.map(str::trim).filter(|id| !id.is_empty()) {
            ...
            if let Some(request_id) = latest {
                request = runtime.running_requests.remove(&request_id);
            }
        }
    }
    if request.is_none() {
        let id = account_id.map(str::trim).unwrap_or_default();
        if id.is_empty() { return; }
        request = Some(CodexLocalAccessRunningRequest { request_id: normalize_activity_request_id(request_id), account_id: id.to_string(), email: account_email.unwrap_or_default().trim().to_string(), api_key_id: api_key_id.unwrap_or_default().trim().to_string(), api_key_label: api_key_label.unwrap_or_default().trim().to_string(), model_id: model_id.unwrap_or_default().trim().to_string(), request_kind, routing_strategy: String::new(), started_at: now, last_seen_at: now });
    }
    let request = request.unwrap();
    let entry = runtime.account_activity.entry(request.account_id.clone()).or_insert_with(|| CodexLocalAccessAccountActivity { account_id: request.account_id.clone(), email: request.email.clone(), ..Default::default() });
    if !request.email.trim().is_empty() { entry.email = request.email; }
    entry.last_finished_at = Some(now);
    entry.last_selected_at = entry.last_selected_at.max(request.started_at);
    entry.last_model_id = request.model_id;
    entry.last_api_key_id = request.api_key_id;
    entry.last_api_key_label = request.api_key_label;
    entry.last_request_kind = request.request_kind;
    entry.routing_strategy = request.routing_strategy;
    entry.recent_request_id = request.request_id;
}

async fn finish_account_activity(request_id: Option<&str>, account_id: Option<&str>, account_email: Option<&str>, api_key_id: Option<&str>, api_key_label: Option<&str>, model_id: Option<&str>, request_kind: CodexLocalAccessRequestKind) {
    let now = now_ms();
    let mut runtime = gateway_runtime().lock().await;
    finish_account_activity_locked(&mut runtime, request_id, account_id, account_email, api_key_id, api_key_label, model_id, request_kind, now);
    prune_account_activity(&mut runtime, now);
    emit_local_access_state_updated();
}

fn build_account_activity_snapshot(runtime: &GatewayRuntime, now: i64) -> (Vec<CodexLocalAccessRunningRequest>, Vec<CodexLocalAccessAccountActivity>) {
    let mut running: Vec<_> = runtime.running_requests.values().filter(|item| now.saturating_sub(item.last_seen_at) <= ACCOUNT_ACTIVITY_RUNNING_STALE_MS).cloned().collect();
    let mut counts = HashMap::new();
    for item in &running { *counts.entry(item.account_id.clone()).or_insert(0u32) += 1; }
    let mut activity: Vec<_> = runtime.account_activity.values().filter_map(|item| {
        let count = counts.get(&item.account_id).copied().unwrap_or(0);
        let recent = item.last_finished_at.unwrap_or(item.last_selected_at);
        if count == 0 && now.saturating_sub(recent) > ACCOUNT_ACTIVITY_RECENT_WINDOW_MS { return None; }
        let mut next = item.clone(); next.running_count = count; Some(next)
    }).collect();
    activity.sort_by(|a,b| b.running_count.cmp(&a.running_count).then_with(|| b.last_selected_at.cmp(&a.last_selected_at)).then_with(|| a.email.cmp(&b.email)));
