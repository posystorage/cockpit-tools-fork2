...
- **Separate Codex context settings from model management**: ordinary accounts follow the official model catalog by default and use the Cockpit catalog only after explicit confirmation. The standalone Context Management dialog offers Follow official, `516K / 460K`, `1M / 900K`, and Custom options for the current instance. Legacy per-model context overrides are removed.
- **Custom ordering for Codex visible models**: drag to reorder models; newly added models follow models from the same source.
- **Preserve Team quota history for Business accounts** ([#2293](https://github.com/jlcodes99/cockpit-tools/pull/2293)): historical quota and reset times remain visible as a local snapshot after switching to usage-based billing, isolated by user and workspace.

### Fixed

- **Support the Homebrew 6 Cask installation flow** ([#2279](https://github.com/jlcodes99/cockpit-tools/pull/2279)): migrate quarantine cleanup to `postflight_steps` and remove the deprecated URL verification argument.
- **Preserve per-execution WebSocket usage accounting** ([#2292](https://github.com/jlcodes99/cockpit-tools/pull/2292)): record and deduplicate usage per execution while retaining account attribution.
- **Fix Antigravity GCP ToS account eligibility and state loss during switching** ([#2286](https://github.com/jlcodes99/cockpit-tools/pull/2286)): prepare metadata, preserve IDE caches, and synchronize credentials to existing data directories.
- **Recover Codex API Service and account pool failures**: add guarded Sidecar process recovery, safer port cleanup, and per-account recovery progress without waiting for durable writes. The Sidecar process-monitoring and port-cleanup changes were reverted in 1.3.44; see that version's notes for the effective behavior.
- **Fix third-party API routing being impossible to disable**: persist an explicit off state without deleting the channel configuration.
- **Fix instant voice under mixed routing**: use the same local gateway and account for call setup and realtime control; avoid restoring active instance profiles on exit.
- **Restore Fast and Ultra for known third-party GPT models** and preserve saved model order.
- **Preserve model-routing drafts and notices** across account refreshes; allow channels without selected models.

## [1.3.42] - 2026-09-07

### Fork synchronization notes

- Merged all upstream changes from `1.3.41` and `1.3.42`; this release does not repeat `1.3.40` or earlier history.
- Accepted upstream `rolling7d` as the default last-seven-days range while retaining 24H and 48H on the standalone API Service page; legacy `last7d` preferences migrate to `rolling7d`, and shared dialogs do not expose 24H/48H.
- Preserved API Service dispatch observability, the complete account list, dispatch-first ordering, and an internal scroll area for large pools; `recordingSelector` remains outside the full upstream selector chain and records affinity cache hits.
- Preserved deleted-account historical billing, the GPT-5.6 Luna pricing policy, Sub2API usage compatibility, ad/remote-config disabling, and the Windows + macOS draft build boundary.
...
- **Fixed Model & Capabilities pricing settings being impossible to save when optional preset values were empty**: cached, long-context, Priority prices, and the token threshold may stay empty when a model does not use them; numeric validation now applies only to entered values.
