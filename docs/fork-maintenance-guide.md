...
5. 在 API 服务启用期间被动观察 Codex OAuth 上游响应的 `X-Codex-Turn-State` 头，只记录 292、312、332、356 四种字符长度各自最近观察到的时间和模型，并在普通 Codex 账号卡及独立 API 服务账号池卡片中展示。

### Turn-State 只读观测边界

- 292、312、332、356 是头值**字符长度**，不是 HTTP 状态码，也不表示令牌有效、限流原因或模型能力。两处卡片统一显示四种长度的最近观察时间；无记录显示 `-`。停用 API 服务时历史记录保留、显示“监听已关闭”，不产生新观测。
...
- 仅保存四种长度的最近元数据到独立 SQLite 表 `codex_turn_state_observations`；不得持久化、打印或传给 UI 原始头值，不得修改客户端或上游的请求头、响应头、token、路由、调度、重试、计费和响应正文。记录时间表示 sidecar 的结果钩子观察时刻；流式长请求可能晚于响应头到达时刻。
...
在 11.3 调度检查之外，还需用合成上游响应分别验证 292、312、332、356：只有 API 服务启用、账号属于集合且 API Key 有权限时两处卡片才更新；停用服务、内部 Key、其他账号、空/未知长度、仅客户端请求头都不得写记录。重试的第二次尝试无头时不能沿用第一次的头；退出重启后最近记录应保留，悬停应显示模型与完整时间，原始头值不得出现在 stdout、SQLite、状态 JSON 或日志。WebSocket 需单独证实现有路径能读到上游握手头，不能为覆盖率主动探测或更改协议。
