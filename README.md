...
1. 禁用广告、赞助推广、远端公告和运行时自动更新。
2. 在 Codex API 服务中显示当前及最近调度账号等只读观测信息。
3. 增强 Sub2API 用量与余额接口兼容。

免责声明：本 fork 仅用于个人研究、学习与本地测试，不建议用于生产或商业用途。除个人研究需要外，建议优先使用原版 Cockpit Tools，并以原作者发布的版本、说明和许可为准。使用、编译、分发或安装本 fork 所产生的风险与责任由使用者自行承担。
